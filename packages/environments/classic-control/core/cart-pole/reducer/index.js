...
module.exports = (constants) => {
  ...
  return (state = initialState, action) => {
    if (action.type === types.left || action.type === types.right) {
      ...
      const angularAcceleration = dynamics.getAngularAcceleration(Object.assign({}, state, { force }));
      const angularVelocity = state.angularVelocity + angularAcceleration * dt;
      const theta = state.theta + angularVelocity * dt;
      const acceleration = dynamics.getAcceleration(Object.assign({}, state, { force, angularAcceleration }));
