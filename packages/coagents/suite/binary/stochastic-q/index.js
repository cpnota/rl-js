...
const Network = require('../../../networks/stochastic-q');
...
module.exports = new AgentBuilder({
  ...
  createAgent: (environmentFactory, hyperparameters) => {
    ...
    const inputV = new LinearStateValue({
      basis,
      alpha: hyperparameters[ALPHA_V],
    });

    const inputQ = new LinearStateValue({
      basis: new Fourier({
    ...
    const outputV = new LinearStateValue({
      basis: outputBasis,
      alpha: hyperparameters[ALPHA_V],
    });

    const outputQ = new TabularActionValue({
      createStateValueFunction: () => new LinearStateValue({
        basis: outputBasis,
        alpha: hyperparameters[ALPHA_V],
      }),
